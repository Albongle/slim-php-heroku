...
class Producto{
    ...
    public function SetId(int $id)
    {
        if(isset($id) && is_int($id)){
            $this->id = $id;
            self::EscribirUltimoID("./ultimoid.txt",$id);
        }else{
            echo "ID invalido\n";
        }
    }
    ...
    public static function NuevoProducto(string $codigo,string $nombre, string $tipo, int $stock, float $precio)
    {
        ...
        $arrayProdcutos = self::LeerArchivo("./productos.json");
        $validacionProducto = $productoAux->ValidarProducto($arrayProdcutos);
        
        if($validacionProducto < 0)
        {
            $productoAux->SetId((self::LeerUltimoID("./ultimoid.txt") + 1));
            if(self::AgregarRegistros($productoAux,"./productos.json")){
        ...
        else if($validacionProducto >= 0)
        {
            ...
            if(self::ModificarRegistros($arrayProdcutos,"./productos.json")){
     ...
     * @return devuelve True, si el producto se encuentra en el Array, False de lo contrario
    */
    ...
    */
    public function Equals(Producto $producto)
    {
        ...
        {
            if(($producto->GetNombre() == $this->GetNombre()) && ($producto->GetCodigoDeBarras() == $this->GetCodigoDeBarras()) && ($producto->GetTipo() == $this->GetTipo()))
            {
                $returnAux = true;
            }
        }
